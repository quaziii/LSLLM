 ...
 */
public class SOARoundRobinPOSolver {
    ...
    private static boolean processarMatch(String pathLog, String arquivo) {
        ...
        try {
            ...
            Logger.getLogger(SOARoundRobinPOSolver.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }
    ...
    private static boolean finalizarSOA(String pathSOA) {
        ...
        File diretorio = new File(pathSOA);
        buscarParcial(diretorio, ".txt", arquivos);
        System.out.println("Total de arquivos na pasta= "+arquivos.size());
        if (arquivos.isEmpty()) {
            return true;
        }
